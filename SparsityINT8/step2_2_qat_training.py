...
def train_qat(args, criterion, data_loader, data_loader_test, data_loader_val, data_loader_calib):
    ...
    try:
        ...
        print(
            "Model definition doesn't accept `quantize` parameter. Instantiating model without quantizing residual connections."
        )
        model_qat = models.__dict__[args.model_name](pretrained=True)
    # quant_modules.deactivate()

    model_qat = model_qat.to(args.device)

    if args.distributed:
        model_qat = torch.nn.parallel.DistributedDataParallel(model_qat, device_ids=[args.gpu])
    else:
        model_qat = torch.nn.DataParallel(model_qat)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--onnx_batch_size", type=int, default=None, help="Batch size for ONNX export.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=68,
        help="Dataset to be used for model calibration." " If None, take the entire val data. Otherwise, take subset.",
    )
    parser.add_argument(
        "--val_data_size",
        type=int,
    ...
    parser.add_argument(
        "--sparse_ckpt",
        type=str,
        default="sparse-finetuned_best.pth",
    ...
    # QAT params
    parser.add_argument("--qat_epoch", type=int, default=10, help="Number of epochs to fine-tune QAT model.")
    parser.add_argument(
        "--qat_steps_per_epoch",
        type=int,
        default=500,
    ...
    parser.add_argument(
        "--qat_ckpt",
        type=str,
        default="quant-finetuned_best.pth",
    ...
    parser.add_argument(
        ...
        default="multistep",
        type=str,
        help="LR Scheduler, options={multistep, step}",
    )
    parser.add_argument(
        "--lr_warmup_method",
        default="constant",
        type=str,
        help="Warmup method, options={constant, linear}",
    )
    parser.add_argument(
        "--lr_warmup_epochs",
        default=1,
    ...
    parser.add_argument(
        "--rewrite_qat_weights",
        dest="rewrite_qat_weights",
        action="store_true",
        help="Rewrite QAT checkpoint if it exists.",
    )
    parser.add_argument(
        "--is_dense_training",
        dest="is_dense_training",
        action="store_true",
    ...
    # by default use the same batch size as one on training
    if args.onnx_batch_size is None:
        args.onnx_batch_size = args.batch_size
